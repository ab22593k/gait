...
impl CacheMetadata {
    pub fn new(config: &RepositoryConfiguration, cache_path: &str, commit_hash: &str) -> Self {
        ...
        // Get the directory size (simplified - just returns 0 for now)
        let size = get_directory_size(cache_path).unwrap_or(0);
...
mod tests {
    ...
    fn test_cache_metadata_manager() {
        ...
        let test_cache_path = cache_path_binding.to_str().unwrap();

        let metadata = CacheMetadata::new(&config, test_cache_path, "abc123");
        let key = "test-key";

        // Store metadata
        manager.store_metadata(key, metadata.clone()).unwrap();

        // Retrieve metadata
        let retrieved = manager.get_metadata(key).unwrap();
        assert_eq!(retrieved.repo_url, metadata.repo_url);

        // Update access time
        manager.update_access_time(key).unwrap();
