...
use crate::cache::{
    fetcher::RepositoryFetcher, filter::RepositoryFilter, key_generator::CacheKeyGenerator,
    manager::CacheManager,
};
use crate::common::{ErrorType, Target, parse};
...
pub fn sync_with_caching(
    target: Target,
    _mode: crate::common::sequence::Mode,
    ...
    for config in &unique_configs {
        ...
        for op in &mut wire_operations {
            if op.source_config.url == config.url && op.source_config.branch == config.branch {
                op.cached_repo_path = cache_path.clone();
            }
        }
    }

    let _filter = RepositoryFilter;
    ...
    for wire_op in &wire_operations {
        ...
        if dest_dir.exists() {
            remove_items(&[dest_dir.as_path()]).map_err(|e| {
                cause!(ErrorType::MoveFromTempToDest)
                    .src(e)
                    .msg(format!("Could not remove {dest_dir:?}"))
            })?;
        }
        ...
        copy_items(&[source_content.as_path()], &dest_dir, &opt).map_err(|e| {
            cause!(ErrorType::MoveFromTempToDest)
                .src(e)
                .msg(format!("Could not copy {source_content:?} to {dest_dir:?}"))
        })?;

        debug!(
            "Copied contents of {} to {}",
            source_subdir, wire_op.source_config.target_path
        );
    }
