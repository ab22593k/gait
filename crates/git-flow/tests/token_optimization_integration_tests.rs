use ritex_flow::commit::prompt::{create_system_prompt, create_user_prompt};
use ritex_flow::config::Config;
use ritex_flow::token_optimizer::TokenOptimizer;
...
async fn test_token_optimization_integration() {
    ...
    // Only check for content if the prompt was NOT truncated (doesn't end with ellipsis)
    if !prompt.ends_with("…") {
        assert!(
